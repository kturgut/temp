...
object CDCDwellEventConsumer {
  ...
  def init(system: ActorSystem[_]): Unit = {
    ...
    val consumerSettings = ConsumerSettings(system, new StringDeserializer, new ByteArrayDeserializer).withGroupId("shopping-cart-analytics")
        ...
        RestartSettings(
          ...
          randomFactor = 0.1)) { () =>
              Consumer
                ...
                .mapAsync(1) { msg => handleRecord(msg.record).map(_ => msg.committableOffset) }
  ...
  private def handleRecord(record: ConsumerRecord[String, Array[Byte]]): Future[Done] = {
    ...
    try {
      ...
      val event =
        typeUrl match {
          case "shopping-cart-service/shoppingcart.ItemAdded" => proto.ItemAdded.parseFrom(inputBytes)

          case "shopping-cart-service/shoppingcart.ItemQuantityAdjusted" =>  proto.ItemQuantityAdjusted.parseFrom(inputBytes)
          case "shopping-cart-service/shoppingcart.ItemRemoved" => proto.ItemRemoved.parseFrom(inputBytes)

          case "shopping-cart-service/shoppingcart.CheckedOut" => proto.CheckedOut.parseFrom(inputBytes)
          case _ => throw new IllegalArgumentException(s"unknown record type [$typeUrl]")
        }
